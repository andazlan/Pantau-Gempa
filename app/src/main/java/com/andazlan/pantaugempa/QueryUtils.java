...
public final class QueryUtils {
    ...
    public static List<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray earthquakeJsonArr = json.getJSONArray("features");
            for (int i = 0; i < earthquakeJsonArr.length(); i++) {
                Earthquake earthquake = new Earthquake(earthquakeJsonArr.getJSONObject(i).getJSONObject("properties"));
    ...
    private static List<Earthquake> extractFeatureFromJson(String jsonEarthquake){
        if (TextUtils.isEmpty(jsonEarthquake)){
            return null;
        }

        List<Earthquake> earthquakeList = new ArrayList<>();
        try {
            JSONObject baseJsonResponse = new JSONObject(jsonEarthquake);
            JSONArray jsonArrayEarthquake = baseJsonResponse.getJSONArray("features");
            for (int i = 0; i < jsonArrayEarthquake.length(); i++) {
                JSONObject currentEarthquake = jsonArrayEarthquake.getJSONObject(i);
                JSONObject properties = currentEarthquake.getJSONObject("properties");
                Earthquake earthquake = new Earthquake(properties);
    ...
    public static URL createUrl(String someUrl){
        URL url = null;
        try {
            url = new URL(someUrl);
        }
